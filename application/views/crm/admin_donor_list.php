                    ...
                    <div class="col-md-12">
                        <div class="portlet">
                            ...
                            <div class="portlet-body">
                                <h5 class="text-danger">Please check the checkbox and then select employees for donor transfers</h5>
                                <div class="row">
                                    <div class="input-group col-sm-4 ">

                                        <select name="user_id" class="form-control select2-1 employee" id="select2-1">
                                            <option value="">Select Employee</option>
                                            <?php
                                            foreach ($employee as $employeeList) {
                                            ?>
                                                <option value="<?= $employeeList['admin_id'] ?>"><?= $employeeList['admin_username'] ?> (<?= $employeeList['admin_contact'] ?>)</option>
                    ...
                    <div class="col-12">
                        <div class="portlet">
                            <div class="portlet-header portlet-header-bordered">
                                ...
                                <span id="stopPaging" class="badge badge-info">View all</span>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
                                    <?php
                                ...
                                <table id="datatable-1" class="table table-bordered table-striped table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                                <tr id="row<?= $donor['id'] ?>">
                                                    <td> <?= $i ?></td>
                                                    <td><input type="checkbox" class="lead_checkbox" data-emp-id="<?= $donor['id'] ?>">
                                                    </td>
                                                    <td><?= convertDatedmy($donor['create_date']) ?></td>
                                                    <td><?= $employee[0]['admin_username'] ?>(<?= $employee[0]['admin_contact'] ?>)</td>
                                                    <td><?= $donor['name'] ?></td>
                                                    <td><?= $donor['email'] ?></td>
                                                    <td><?= $donor['number'] ?></td>
                                                    <td><?= $donor['address'] ?></td>
                                                    <td> <span class="badge badge-<?= (($donor['status'] == 'NEW') ? 'info' : (($donor['status'] == 'Repeated Donor') ? 'success' : (($donor['status'] == 'Not Interested') ? 'danger' : (($donor['status'] == 'Followup') ? 'warning' : 'primary'
                                                                                    ))))
